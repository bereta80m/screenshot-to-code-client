				...
				<section className="max-w-5xl w-full mx-auto p-10">
					...
					{step === STEPS.INITIAL && (
						<div className="flex flex-col gap-4">
							<DragDropComponent transformImageToCode={transformImageToCode} />
							<FormComponent transformUrlToCode={transformUrlToCode} />
						</div>
					)}
...
export default function Home() {
  ...
  const TransformUrlToCode = async (url: string) => {
    setSteps(STEPS.LOADING)
    const response = await fetch('/api/generate-code-from-image', {
      method: 'POST',
    ...
    setSteps(STEPS.PREVIEW)

    const reader = response.body.getReader()
    const decoder = new TextDecoder()

    //Since we do not know how much data is going to be written we need to create a while
    while (true) {
      const { done, value } = await reader.read()
      const chunk = decoder.decode(value)
      ...
      //When it finishes it will stop the infinite loop
      if (done) break;
    }
  }
*/
